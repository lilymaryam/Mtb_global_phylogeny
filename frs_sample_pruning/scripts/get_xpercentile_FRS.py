...
def main():
    parser = argparse.ArgumentParser(
        description='Calculate percentile threshold per ratio column'
    )
    ...
    parser.add_argument('-p', '--percentile', type=float, default=0.99,
                        help='Percentile threshold (default: 0.99)')
    ...
    pct_label = f"p{int(args.percentile * 100)}"

    stats = pd.DataFrame({
        pct_label: df.quantile(args.percentile),
        'n': df.count(),
    })
